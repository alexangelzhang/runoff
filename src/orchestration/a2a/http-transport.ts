...
export interface HttpA2ATransportOptions {
  host?: string;
  port?: number;
  /** Phase 7.9 production: optional bearer auth on inbound requests. */
  auth?: A2AHttpAuthConfig;
  /** Service discovery: GET /a2a/agents lists registered cards. */
  registry?: AgentCardRegistry;
  /** External discovery URLs merged into GET /a2a/agents. */
  remoteDiscoveryUrls?: string[];
  /** Outbound Bearer token for send() and remote discovery. */
  clientToken?: string;
  /** HTTPS server mTLS/TLS (cert + key required). */
  tls?: A2AServerTlsConfig;
  /** HTTPS client mTLS for send() and remote discovery. */
  clientTls?: A2AClientTlsConfig;
  /** Cache remote discovery results (ms). */
  remoteDiscoveryTtlMs?: number;
  /** Persist remote agents to federation store (default on). */
  ...
  /** P8: background lease renewal (default on when lease enabled). */
  federationLeaseHeartbeat?: boolean;
  federationLeaseHeartbeatMs?: number;
  /** P9: peer URLs to witness lease (split-brain check on sync). */
  ...
  federationSkillDepsPruneStrategy?: import("./federation-skill-deps.js").SkillDepPruneStrategy;
  ...
  /** P9: tombstone GC retention ms (default 7d; 0 = off). */
  federationTombstoneRetentionMs?: number;
  /** P11: skill tombstone GC retention (defaults to federationTombstoneRetentionMs). */
  ...
  private async handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (url.pathname === "/a2a/federation/lease/witness" && req.method === "POST") {
        ...
        const receipt = recordRemoteLeaseWitness(entry, this.options.federationPath);
        res.writeHead(200, { "content-type": "application/json" });
      ...
      if (url.pathname === "/a2a/federation/lease/witnesses") {
        if (req.method === "GET") {
          ...
          const log = readLeaseWriteWitnessLog(this.options.federationPath);
          res.writeHead(200, { "content-type": "application/json" });
      ...
      if (url.pathname === "/a2a/federation/lease/audit/export") {
        if (req.method === "GET") {
          ...
          const chain = readLeaseAuditChain(this.options.federationPath);
          const format = url.searchParams.get("format") ?? "json";
          if (format === "bundle" || format === "manifest") {
            const keyRing = readLeaseAuditKeyRing(this.options.federationPath);
            const auditKid = this.options.federationLeaseAuditKeyId ?? keyRing.activeKeyId;
            const secrets: Record<string, string> = {
              ...(this.options.federationLeaseAuditKeyRing ?? {}),
            };
            if (this.options.federationLeaseAuditSecret) {
              secrets[auditKid] = this.options.federationLeaseAuditSecret;
            }
            const secret = secrets[keyRing.activeKeyId] ?? this.options.federationLeaseAuditSecret;
      ...
      if (url.pathname === "/a2a/federation/lease/audit/keyring") {
        if (req.method === "GET") {
          ...
          const ring = readLeaseAuditKeyRing(this.options.federationPath);
          res.writeHead(200, { "content-type": "application/json" });
      ...
      if (url.pathname === "/a2a/federation/skill-deps/prune-log") {
        if (req.method === "GET") {
          ...
          const log = readSkillDepsPruneLog(this.options.federationPath);
          const limitRaw = url.searchParams.get("limit");
          const limit = limitRaw ? Number.parseInt(limitRaw, 10) : undefined;
      ...
      if (url.pathname === "/a2a/federation/lease") {
        if (req.method === "GET") {
          ...
          const lease = readFederationLease(federationLeasePath(this.options.federationPath));
          res.writeHead(200, { "content-type": "application/json" });
      ...
      if (url.pathname === "/a2a/federation/directory") {
        if (req.method === "GET") {
          ...
          const agents = loadFederatedAgentCards(this.options.federationPath);
          res.writeHead(200, { "content-type": "application/json" });
          res.end(JSON.stringify(buildFederationDirectoryBody(agents)));
          return;
        }
        if (req.method === "POST" && this.isAuthorized(req)) {
          const raw = await readBody(req);
