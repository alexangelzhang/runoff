...
import { agentId as toAgentId } from "../../orchestration/multi-agent-types.js";
import {
...
export function deleteFederatedAgentCard(
  ...
  const aid = toAgentId(agentId);
  const existing = local.find((c) => c.agentId === aid);
  const tomb = createAgentCardTombstone(aid, nodeId, existing);
