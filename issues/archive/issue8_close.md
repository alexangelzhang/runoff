...
**File:** `src/observability/trace.ts` — used by `tests/unit/trace.test.ts`. Kept as-is.
...
**File:** `src/orchestration/multi-agent-types.ts` — used by `tests/unit/multi-agent-types.test.ts`. Kept as-is.
