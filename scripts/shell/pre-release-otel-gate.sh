...
#   - Optional org collector URL (no local start): vars.RUNOFF_OTEL_ENDPOINT
...
if [[ -n "${RUNNER_TOOL_CACHE:-}" ]]; then
  export RUNOFF_HOME="${RUNOFF_OTEL_CACHE_DIR:-$RUNNER_TOOL_CACHE/runoff-otel}"
else
  export RUNOFF_HOME="${RUNOFF_OTEL_CACHE_DIR:-${RUNOFF_HOME:-$HOME/.runoff}/pre-release-otel}"
fi
mkdir -p "$RUNOFF_HOME/bin"

export RUNOFF_OTEL_DOWNLOAD="${RUNOFF_OTEL_DOWNLOAD:-1}"
export RUNOFF_OTEL_COLLECTOR_REQUIRED="${RUNOFF_OTEL_COLLECTOR_REQUIRED:-1}"
export RUNOFF_OTEL_RECLAIM_PORT="${RUNOFF_OTEL_RECLAIM_PORT:-1}"

# Repository variable: corporate / shared collector (offline-friendly, no GitHub download).
if [[ -n "${RUNOFF_OTEL_ENDPOINT:-}" ]]; then
  export OTEL_EXPORTER_OTLP_ENDPOINT="${RUNOFF_OTEL_ENDPOINT}"
  ...
  export RUNOFF_OTEL_SKIP_START="${RUNOFF_OTEL_SKIP_START:-0}"
fi

echo "pre-release-otel: RUNOFF_HOME=$RUNOFF_HOME"
...
precheck() {
  ...
  if [[ -x "$RUNOFF_HOME/bin/otelcol-contrib" ]]; then
    echo "pre-release-otel: cached binary at $RUNOFF_HOME/bin/otelcol-contrib"
    return 0
  fi
  if [[ "${RUNOFF_OTEL_DOWNLOAD:-0}" == "1" ]] && command -v curl &>/dev/null; then
    echo "pre-release-otel: will download otelcol-contrib on first start"
    return 0
  fi
  echo "pre-release-otel: FAIL — no collector binary, curl, or RUNOFF_OTEL_ENDPOINT" >&2
  echo "  Fix: set repository variable RUNOFF_OTEL_ENDPOINT, or install otelcol-contrib on runner, or ensure curl for download." >&2
...
resolve_collector_bin_precheck() {
  local name
  [[ -n "${RUNOFF_OTEL_BIN:-}" && -x "${RUNOFF_OTEL_BIN}" ]] && return 0
  [[ -x "$RUNOFF_HOME/bin/otelcol-contrib" ]] && return 0
...
on_fail() {
  ...
  if [[ -f "$RUNOFF_HOME/otel-collector.log" ]]; then
    echo "pre-release-otel: last 40 lines of log:" >&2
    tail -n 40 "$RUNOFF_HOME/otel-collector.log" >&2 || true
  fi
}
