...
9. **Harness evolution:** failure signatures, candidate manifests, proposer outputs, history contexts, observed variant diffs, held-in/held-out gates, rank decisions, acceptance checks, promotion bundles, and rollback records are audit artifacts under `~/.runoff/harness-evolution/`. Automatic proposers must run with `workDir` set to the candidate `variant/` directory and should consume mined failure signatures/history context before editing; proposal audit must compare provider-reported files with observed variant changes; accept decisions must require a clean proposal, observed diff, and passing held-in/held-out gate. Promotion export copies accepted variant files into an audit bundle only; it must not apply edits to user repos from TypeScript. Keep repo mutation in the existing workspace/runtime paths.
