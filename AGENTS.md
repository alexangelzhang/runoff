...
9. **Harness evolution:** candidate manifests, proposer outputs, observed variant diffs, held-in/held-out gates, rank decisions, acceptance checks, and rollback records are audit artifacts under `~/.runoff/harness-evolution/`. Automatic proposers must run with `workDir` set to the candidate `variant/` directory; proposal audit must compare provider-reported files with observed variant changes; accept decisions must require a clean proposal, observed diff, and passing held-in/held-out gate. Do not auto-apply edits to user repos from TypeScript. Keep repo mutation in the existing workspace/runtime paths.
