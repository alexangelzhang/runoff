...
9. **Harness evolution:** candidate manifests, proposer outputs, observed variant diffs, held-in/held-out gates, rank decisions, and rollback records are audit artifacts under `~/.runoff/harness-evolution/`. Automatic proposers must run with `workDir` set to the candidate `variant/` directory; proposal audit must compare provider-reported files with observed variant changes and must not auto-apply edits to user repos from TypeScript. Keep repo mutation in the existing workspace/runtime paths.
