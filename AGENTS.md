...
9. **Harness evolution:** candidate manifests, proposer outputs, held-in/held-out gates, rank decisions, and rollback records are audit artifacts under `~/.runoff/harness-evolution/`. Automatic proposers must run with `workDir` set to the candidate `variant/` directory and must not auto-apply edits to user repos from TypeScript; keep repo mutation in the existing workspace/runtime paths.
