...
import { applyRaceSession, abortRaceSession } from "../../src/runtime/race-finalize.js";
import { recordTrace } from "../../src/observability/trace.js";
...
import { REPO_ROOT as ROOT_DIR } from "../helpers/repo-root.js";
...
test("runPipelineMode agent race auto-pick applies winner without awaiting_judge", async () => {
  ...
  const winnerScript = createExecutableScript(
    sandboxDir,
    "pipeline-race-autopick-winner.sh",
    "#!/bin/sh\nprintf 'winner line\\n' >> module.txt\n",
  );
  const loserScript = createExecutableScript(
    sandboxDir,
    "pipeline-race-autopick-loser.sh",
    "#!/bin/sh\nprintf 'loser line\\n' >> module.txt\n",
  );
  ...
  try {
    ...
    assert.notEqual(result.status, "awaiting_judge");
    assert.equal(readFileSync(join(repoDir, "src", "module.txt"), "utf-8"), "base module\nwinner line\n");
    assert.equal(raceSessions.has(result.traceId), false);
  } finally {
    activeWorkspaces.clear();
