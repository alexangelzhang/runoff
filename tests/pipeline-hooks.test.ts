...
describe("composeEffectivePipelineContext", () => {
  it("appends pattern context when both are present", () => {
    assert.equal(
...
describe("PipelineHooks.onPipelineEnd", () => {
  it("tags trace with experiment metadata", async () => {
    ...
    hooks.onPipelineEnd({ trace, costTracker: new CostTracker() });
  ...
  it("same experimentId for same prompt", async () => {
    ...
    hooks1.onPipelineEnd({ trace: trace1, costTracker: new CostTracker() });
    ...
    hooks2.onPipelineEnd({ trace: trace2, costTracker: new CostTracker() });
  ...
  it("onPipelineFailed records experiment without baseline judge", async () => {
    ...
    hooks.onPipelineFailed({ trace, costTracker: new CostTracker() });
  ...
  it("onPipelineEnd attaches costSummary", async () => {
    ...
    hooks.onPipelineEnd({ trace, costTracker: tracker });
