...
import { getPipelineMemory, resetPipelineMemoryRegistry } from "../../src/memory/pipeline-memory.ts";
import { resolveMemoryBackendConfig } from "../../src/orchestration/memory-factory.ts";
...
test("getPipelineMemory http uses LayeredAgentMemory when URL set", () => {
  const config: PipelineConfig = {
    providers: { m: { type: "mock" } },
    pipeline: { s: ["m"] },
    orchestration: {
  ...
  const mem = getPipelineMemory(config);
