...
| Tests                     | ~876 pass, 6 skip (`npm run test:ci`)                                                                           |
...
| Harness evolution substrate | `runoff_harness_evolve` · `src/orchestration/harness-evolution.ts` · `src/orchestration/harness-artifact-store.ts` · trigger scans, failure signature mining, taskset/verifier registry, trajectory/replay contracts, training-ready trajectory export, paddock adapter registry, sandbox lease abstraction, rollout batch scheduler, reward registry/report, rule/skill registry, prompted feedback compiler, harness GC loop, autonomy policy gate, context topology/router, artifact store index/doctor, dataset/split object, dataset/taskset evaluation, persisted plan/run/report, full evolution-loop orchestration, role policy evidence, local connector writeback, change manifest, candidate lineage, isolated proposer, rejected-buffer history context, observed variant diff, leakage/overfit audit, held-in/out gate, skill patch gate, variant isolation, coreset, self-preference rank, persisted frontier, acceptance guard/rollback audit, promotion bundle export |
...
| --------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Core runtime          | Production-ready local path       | `npm run ci:gates` covers IPC sync, typecheck, gate2/gate3, example doctor, and full tests.                                                                      |
| Harness control plane | Implemented local audit artifacts | Artifact creation, reports, gates, decisions, rollback records, promotion bundles, artifact indexes, and doctor reports are local durable/control-plane state under `~/.runoff/harness-evolution/`. |
| Adapter contracts     | Contract-ready                    | Paddock, sandbox, rollout, training export, and connector objects are stable contracts; remote lifecycle and blackbox execution remain behind explicit adapters. |
| Optional systems      | Opt-in                            | A2A, external memory, Dream/Dreamify, OTel collector, and real-provider smoke are not required for the core path.                                                |
