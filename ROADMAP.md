...
| **P0 开源发布包** `DONE` | MIT · 贡献指南 · `npm run demo` · `pipeline:run` · 多 coding-agent 后端文档（**非** Docker/devcontainer） | [`docs/OPEN_SOURCE.md`](docs/OPEN_SOURCE.md) · [`LICENSE`](LICENSE) · [`CONTRIBUTING.md`](CONTRIBUTING.md) · [`CHANGELOG.md`](CHANGELOG.md) · [`examples/`](examples/) · [`scripts/ts/dev/pipeline-cli.ts`](scripts/ts/dev/pipeline-cli.ts) |
