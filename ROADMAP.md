...
| Harness evolution substrate | `runoff_harness_evolve` · `src/orchestration/harness-evolution.ts` · trigger scans, failure signature mining, dataset/split object, dataset evaluation, persisted plan/run/report, full evolution-loop orchestration, role policy evidence, local connector writeback, change manifest, candidate lineage, isolated proposer, history context, observed variant diff, leakage/overfit audit, held-in/out gate, variant isolation, coreset, self-preference rank, persisted frontier, acceptance guard/rollback audit, promotion bundle export |
