...
| Harness evolution substrate | **Extracted to the standalone `agent-evolution` project** (host-agnostic `propose → evaluate → accept` loop: trigger scans, failure signature mining, taskset/verifier registry, trajectory/replay, training export, paddock/sandbox/rollout/reward/rule/feedback/GC/autonomy/context-topology, dataset splits, orchestrated runs, role policy, isolated proposer, observed variant diff, leakage audit, held-in/out gate, skill patch gate, rank/frontier, acceptance guard, promotion bundle). No longer part of runoff. |
...
| Harness control plane | Implemented local audit artifacts | Artifact creation, reports, gates, decisions, rollback records, promotion bundles, artifact indexes, and doctor reports live in the standalone `agent-evolution` project (formerly `~/.runoff/harness-evolution/`). |
