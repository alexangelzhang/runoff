# Delivered phases (Phase 0–8, gates, benchmarks detail)

> Archived from root [ROADMAP.md](../../ROADMAP.md) on 2026-05-26. **Current status** lives in the slim ROADMAP; this file is historical reference only.
...
- 1.3 等价性 CI 全绿（`tests/unit/pipeline-execution-equivalence.test.ts`）
...
- 2.3 config 声明式 governance 规则模板（`runtime.governance.rules` 文档化） — `docs/architecture/governance-config.md`, `config.ts` 校验
...
- 6.1 Real Provider Smoke Tests — `tests/integration/real-provider.integration.test.ts` + `scripts/ts/dev/run-real-provider-smoke.ts` + nightly/pre-release workflows + `tests/unit/real-provider-smoke.test.ts`（runner 契约）
- 6.2 Orchestration 集成测试（workflow 路径 + CP crash recovery） — `tests/unit/orchestration-e2e.test.ts`
...
**当前优先级**：Backlog 已清空；新需求见 [ROADMAP Future](../../ROADMAP.md#future)。
...
触发条件：Phase 7.2 完成后。验收：`npm run test:gate2`（`tests/e2e/gate2-control-plane.e2e.test.ts`）。
...
触发条件：Phase 7.3 + 7.4 + 7.8 完成后。验收：`npm run test:gate3`（`tests/e2e/gate3-orchestrator.e2e.test.ts`）。
...
**完整双层对标（战略概念 + 战术代码路径 @ pinned SHA）：** [docs/reference/industry-benchmark.md](docs/reference/industry-benchmark.md)  
**版本钉扎：** [docs/reference/benchmark-pins.json](docs/reference/benchmark-pins.json) — 刷新：`./scripts/refresh-benchmark-pins.sh`
...
能力勾选完整度约 **95%+**（战略）。战术对齐度以 [industry-benchmark.md](docs/reference/industry-benchmark.md) 为准。Backlog **B2–B8** 均已落地。
