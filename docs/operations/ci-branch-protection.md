...
- **No** `RUNOFF_REAL_CODEX_ARGV_JSON` secret → smoke runs with `--allow-skip` (green if CLIs missing).
- **With** `RUNOFF_REAL_CODEX_ARGV_JSON` set → CI runs `npm run ci:gates:smoke:strict` (no allow-skip). Runner must still have `codex`/`gemini` on PATH or the job fails.
...
| Variable | Purpose |
|----------|---------|
| `RUNOFF_REAL_TIMEOUT_MS` | Real-provider smoke timeout |
| `RUNOFF_OTEL_ENDPOINT` | Corporate OTLP HTTP URL — pre-release skips local collector download (offline runner) |
...
- **OTel pre-release gate** (one of): `curl` for first-time binary download into `RUNNER_TOOL_CACHE/runoff-otel`; **or** pre-installed `otelcol-contrib` on PATH; **or** set `RUNOFF_OTEL_ENDPOINT` to an internal collector
