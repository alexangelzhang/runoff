...
```bash
node --import tsx scripts/run-real-provider-smoke.ts --mode manual --allow-skip
node --import tsx scripts/run-real-provider-smoke.ts --mode nightly --report-dir tmp/real-provider-smoke/manual-check
```
...
export LLM_PIPELINE_REAL_GEMINI_ARGV_JSON='["gemini"]'
