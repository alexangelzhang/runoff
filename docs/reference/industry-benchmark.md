...
> **审计日期：** 2026-05-26  
> **版本钉扎：** [`benchmark-pins.json`](reference/benchmark-pins.json)（可用 `./scripts/refresh-benchmark-pins.sh` 刷新）  
> **关联：** [ROADMAP.md](../ROADMAP.md)（executive summary）、[execution-layers.md](../architecture/execution-layers.md)
...
- **LangSmith / Phoenix**：观测平台，与 `src/observability/trace.ts` 对标，不纳入编排五层链。
...
2. **工程护城河**：`task_runner.py` + `workspace_manager.py` + race finalize 与编排层正交（见 [execution-layers.md](architecture/execution-layers.md)）。
...
| 单步 / race | `src/runtime/scheduler.ts`, `src/runtime/race-execution.ts` |
...
4. **可选 E3：** 在 `tests/e2e/gate3-orchestrator.e2e.test.ts` 旁增加「能力探针」清单（不替代 E2）。
